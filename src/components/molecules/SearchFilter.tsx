...
export default function AestheticSearchBar(props: SearchBarProps) {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div
                className="bg-white shadow-lg rounded-full p-2 sm:p-4 flex flex-col sm:flex-row gap-4 sm:gap-2 items-center">
                ...
                <div className="flex w-full sm:w-auto justify-between sm:justify-start gap-2">
                    <Dialog open={isOpen} onOpenChange={setIsOpen}>
                        <DialogTrigger asChild>
                            <Button variant="outline"
                                    className="w-full sm:w-auto text-base py-3 px-6 rounded-full border-2 border-primary hover:bg-primary hover:text-white transition-colors">
                        ...
                        <DialogContent className="sm:max-w-[425px]">
                            ...
                            <RadioGroup value={selectedLocation.id} onValueChange={handleLocationSelect}
                                ...
                                {locations.map((location) => (
                                    <div key={location.id}
                                        ...
                                        <Label htmlFor={location.id}
                                               className="text-lg font-medium cursor-pointer flex-grow">{location.name}</Label>
                                    </div>
                                ))}
                            </RadioGroup>
                        </DialogContent>
                    ...
                    {/*        className="w-full sm:w-auto text-base py-3 px-8 rounded-full bg-primary hover:bg-primary/90 transition-colors">*/}
                    {/*    Filter*/}
                    {/*</Button>*/}
                </div>
            </div>
